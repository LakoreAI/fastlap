...
///
/// Padding with f64::INFINITY (rather than max+1) keeps the padded entries strictly
...
pub fn solve(matrix: Vec<Vec<f64>>) -> LapSolution {
    ...
    // Use a large-but-finite sentinel so arithmetic in sap_solve never produces NaN.
    let fill = matrix
        .iter()
        .flatten()
        .cloned()
